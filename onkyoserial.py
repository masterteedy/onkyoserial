...
class onkyoserial:
  ...
  def disp_info_format(self):
    """Display program format. (Check disp_mode_* if this does not work)"""
    self.ser_cmd('DIF00')

  def disp_info_digital_input(self):
    """Display digital input position. (Check disp_mode_* if this does not work)"""
    self.ser_cmd('DIF01')

  def disp_info_digital_format(self):
    """Display digital format position. (Check disp_mode_* if this does not work)"""
    self.ser_cmd('DIF02')

  def disp_info_bass(self):
    """Display bass level. (Check disp_mode_* if this does not work)"""
    self.ser_cmd('DIF03')

  def disp_info_treble(self):
    """Display treble level. (Check disp_mode_* if this does not work)"""
    self.ser_cmd('DIF04')

  def disp_mode_volume(self):
    """Display volume mode / sets Selector. (Check disp_info_* if this does not work)"""
    self.ser_cmd('DIF0')

  def disp_mode_listening(self):
    """Display listening mode / sets Selector. (Check disp_info_* if this does not work)"""
    self.ser_cmd('DIF0')

  def disp_mode_digital_format(self):
    """Display digital format. (Check disp_info_* if this does not work)"""
    self.ser_cmd('DIF02')

  def disp_mode_video_format(self):
    """Display video format. (Check disp_info_* if this does not work)"""
    self.ser_cmd('DIF03')
